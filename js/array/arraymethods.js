var arr=[10,20]
//properties
//1) length => returns number of items in array
//console.log(arr.length)


//methods
//insertion
//1) push  => arr.push(item)  //insert item at the end
arr.push("abcd")


//2)unshift  => arr.unshift(item)  //insert item at the beginning
arr.unshift("harshu")
console.log(arr)


//3)Remove
//pop() => removes item from last
console.log(arr.pop())
    console.log(arr)

//4)shift()==> removes item from the beginning
console.log(arr.shift())
console.log(arr)

//5)splice==>removes or adds item(s) for spacific location(s)
arr.splice(2,2,"harshita" ,"patidar")
console.log(arr)

//6)looping
//forEach => iterate over array
console.log("foreach")
arr.forEach((ele,index,loopingArray)=>{
    console.log(ele,index,loopingArray)
})


//7)map==> return a new array with return values of given function

var newArr=arr.map((el)=>{
    return el*el
})
var ages = [10, 20, 15, 18, 21, 10, 10]
console.log(newArr,ages)

//8)filter => returns a new array having values for which given function retured true.

var filteredArr = ages.filter((el)=>{
    return el >= 18
 })
 
 console.log(filteredArr)

 //9)reduce==>
 var res=ages.reduce((prevValue,currentValue)=>{
     return prevValue+currentValue
 })
 console.log(res)


 //10)indexof==>
 console.log(ages.indexOf(10, ages.indexOf(10)+1))

 //11)lastIndexof==>how to create copy of an array ?
var students = ["Yahiya", "Vishal", "Ved Prakash", "Siddarth"]
var cpyStudents = [...students] //spread Operator

cpyStudents[0] = "suyash"
console.log(students, cpyStudents)

//12)slice
var copyStu = students.slice()
copyStu[0] = "harshi"
console.log(students, copyStu)


//some  => 
//every => 

var isAnyVoterAvl = ages.some(el=>el>=18)

var isEveryVoterAvl = filteredArr.every(el=>el>=18)

console.log(isAnyVoterAvl, isEveryVoterAvl)

var nmsarr = [1, 2, 3, 4, 5]

nmsarr.reduce((tmp, el)=>{
    console.log("tmp :", tmp, " Elem : ", el )

    return tmp+el
})




//13)sort()==>
var arr=[2,1,30,9,40,74]
var sorted=arr.sort((a,b)=>{
    return b-a
})
console.log(sorted)
var maxsumsubArray=sorted.slice(0,4)
console.log(maxsumsubArray)
